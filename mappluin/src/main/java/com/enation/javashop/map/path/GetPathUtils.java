package com.enation.javashop.map.path;
...
import com.enation.javashop.map.utils.SchemeUtil;
...
public class GetPathUtils {
     ...
     * @param type    类型
     */
    private void initListener( final Listener.DataLisener<PathModel> getPath,@MapType.MT final int type){
        //设置监听
        routeSearch.setRouteSearchListener(new RouteSearch.OnRouteSearchListener() {
            ...
            public void onBusRouteSearched(BusRouteResult busRouteResult, int i) {
                ...
                if (i == 1000) {
                    ...
                    //通知接口数据获取成功
                    getPath.success(pathModel);
                }else{
                    getPath.faild("数据获取失败，ErrorCode:"+i);
                }
            }
            //获取驾驶数据
            @Override
            public void onDriveRouteSearched(DriveRouteResult driveRouteResult, int i) {
                if (type!=MapType.DRIVE){
                    return;
                }
                if (i!=1000){
                    getPath.faild("数据获取失败，ErrorCode:"+i);
            ...
            /**
             * 获取步行数据
             * @param walkRouteResult 返回数据
             * @param i               数据标识
             */
            @Override
            public void onWalkRouteSearched(WalkRouteResult walkRouteResult, int i) {
                if (type!=MapType.WALK){
                    return;
                }
                if (i!=1000){
                    getPath.faild("数据获取失败，ErrorCode:"+i);
